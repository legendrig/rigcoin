...
#if defined(NDEBUG)
# error "Rigcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd192d9b0a28b71071a51cab7a5ca26cedeaee230d4f8604d6b7027e8bb24c358");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rigcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rigcoin Signed Message:\n";
...
{
    // Rigcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 285.38 * COIN;
	
	if(nHeight <= 105200)  
	{
		nSubsidy = 285.38 * COIN;
	}
	else if(nHeight <= 210400)  
	{
		nSubsidy = 190.25 * COIN;
	}
	else
	{
		nSubsidy = 171.23 * COIN;
	}

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6 * 60; // Rigcoin: 6 minutes
static const int64 nTargetSpacing = 1 * 60; // Rigcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rigcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rigcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd192d9b0a28b71071a51cab7a5ca26cedeaee230d4f8604d6b7027e8bb24c358");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CBS News 09/Oct/2015 Where has Hillary Clinton broken with Obama?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041681810fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1444390658;
        ...
        assert(block.hashMerkleRoot == uint256("0x1258254783b72a4c8722a15d1a8a78f845abeebd253f8ae9d431779d0fbd4173"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb4, 0xdd }; // Rigcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RigcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RigcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RigcoinMiner(CWallet *pwallet)
{
    printf("RigcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rigcoin-miner");
    ...
    try  loop {
        ...
        printf("Running RigcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RigcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RigcoinMiner, pwallet));
}
